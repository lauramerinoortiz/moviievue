...
export function VistaListado(controlador) {
     return VistaListado=Vue.createApp({
          ...
          <h1 v-if=(lista!='')>Listado de películas</h1>
          <div v-for="peli in lista" class="pelicula" v-on:click="pulsarPeli(peli.nombre)" v-bind:style="{ 'background-image': 'url(' + peli.imagen + '), url(assets/logo/logosimple.png)'}">
          <div class="oculto"></div>
          <h2>{{peli.nombre}}</h2></div>
          </div>`,
          methods: {
                /**
